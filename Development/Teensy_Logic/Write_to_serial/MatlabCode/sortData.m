function [Data] = sortData(filename,v)
%sortData Sorts data from arduino file
filename = 'Data1.dat';
v{1}     = 'x';
v{2}     = 'y';

% Read in data
D = csvimport('Data1.dat');

if length(v) > width(D)-1
    error('Too many vars given')
elseif length(v) < width(D)-1
    error('Too few vars given')
else
    disp('Good')
end
...
% No data pts
Data.nPts = length(D)-1;
inc = Data.nPts;
%% Sort Clocktime
Data.TimeStart = tConvert(D{1,1})
for i = 2:length(D)
    Data.ClockTime(i-1) = tConvert(D{i,1});
end

%% Sort runtime
for i = 1:inc
    Data.t(i) = seconds(Data.ClockTime(i) - Data.ClockTime(1));
end

%% Sort Vars
for j = 1:length(v)
    Data.(v{j}) = cell2mat(D(2:inc+1,j+1));
end
...
%tConvert Converst time from char to useable format
tOut = tIn(1:12);
tOut = datetime(tOut,'Format','HH:mm:ss,SSS', 'InputFormat', 'HH:mm:ss.SSS')
end
