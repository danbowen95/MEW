...
void A5StartAC() {
  // Start A5 active control

  // Read what new static val is
  A5staticVal = A5AvgSensorVal(100);

  // Now put into AC
  machineStatus.mode   = 'w';
}

void A5EndAC() {

  // Just make sure we've stopped moving
  DriveToSpeed( 0 , 5 );
}

void A5Lift2AC() {
  // Lift then put into AC

  float ValRead;    // The read in val
  float ValMapped;  // The mapped val

  // Define new static val
  A5staticVal = A5AvgSensorVal(100);
  ...
  // Keep moving up until sensor triggered, or at top
...
void A5ActiveControl() {
  // This drives AC around 'A5StaticVal'

  int ValRead;               // Value read by sensor
  int ValMapped;             // PWM value sent to driver
  bool Good2Move = false;    // Checks to see if we can move
  bool AtTop = false;        // Within lims
  bool AtBot = false;
  ...
  if (Good2Move) {
    // Good to move (homed & within lims), move!
    ...
    // We're not good to move, so stop moving:
    DriveToSpeed( 0 , 5 );
  }
}
...
void A8StartAC() {
  // Start A8 active control

  // Read what new static val is
  A8staticVal = A8AvgSensorVal(100);

  // Now put into AC
  machineStatus.mode   = 'x';
}
...
void A8ActiveControl() {
  // This drives AC around 'A8StaticVal'
